...
```{r}
GO <- topTable(fit.ns,coef=8:12,p.value=0.05, n=Inf)
gnames <- rownames(GO)

GO <- data.frame(substr(gnames, 1, nchar(gnames) - 5))

write.csv(GO, file = 'the_genes.csv')
```
...
write.csv(GO, file = 'the_genes3.csv')
...
terms <- GO_terms[,c(1,2)]

write.csv(terms, file = 'terms.csv')

target_list = terms[terms$Gene_id %in% GO$substr.gnames..1..nchar.gnames....5.,]

write.csv(target_list, file = 'target_list.csv')
```
